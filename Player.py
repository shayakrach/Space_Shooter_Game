...
class Player(Ship):
    ...
    def health_bar(self, window):
        red_rect = (self.x, self.y + self.ship_img.get_height() + 10, self.ship_img.get_width(), 10)
        pygame.draw.rect(window, RED, red_rect)
        part_of_health = self.ship_img.get_width() * (self.health / self.max_health)
        if part_of_health > 0:
            green_rect = (self.x, self.y + self.ship_img.get_height() + 10, part_of_health, 10)
            pygame.draw.rect(window, GREEN, green_rect)

    def shoot(self):
        if self.cool_down_counter == 0:
            if self.shooter_type == 'double_shooter':
                laser = Laser(self.x + 30, self.y, self.laser_type)
                self.lasers.append(laser)
                laser = Laser(self.x - 30, self.y, self.laser_type)
                self.lasers.append(laser)
            else:
                laser = Laser(self.x, self.y, self.laser_type)
                self.lasers.append(laser)
