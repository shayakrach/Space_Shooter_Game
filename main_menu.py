...
def main_menu():
    ...
    while run:
        ...
        # Name input
        text_surface = TITLE_FONT.render(user_text, True, WHITE)
        WIN.blit(text_surface, (middle_width + 270, middle_height - 100))
        # input_rect.w = max(180, text_surface.get_width() + 10)

        # Labels
        name_label = TITLE_FONT.render("Enter your name: ", 1, WHITE)  # Create begin label
        WIN.blit(name_label, (middle_width, middle_height - 100))  # Display label in the middle
        ...
        WIN.blit(start_game_label, (middle_width, middle_height))  # Display label in the middle
        ...
        WIN.blit(difficulty_game_label, (middle_width, middle_height +100))  # Display label in the middle
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in [pygame.K_9, pygame.K_8, pygame.K_7, pygame.K_6, pygame.K_5, pygame.K_4, pygame.K_3, pygame.K_2, pygame.K_1]:
                    new_record = Game.start(user_text, event.unicode)
                    after_game_menu(new_record)
                    run = False
                if len(user_text) < 20:
                    if event.unicode.isalpha() or event.type == pygame.K_SPACE:
...
def after_game_menu(record):
    ...
    while run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
            if event.type in [pygame.K_9, pygame.K_8, pygame.K_7, pygame.K_6, pygame.K_5, pygame.K_4, pygame.K_3, pygame.K_2, pygame.K_1]:
                new_record = Game.start(user_text)
